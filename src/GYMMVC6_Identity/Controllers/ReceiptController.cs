...
namespace GYMONE.Controllers
{
    ...
    public class ReceiptController : Controller
    {
        ...
        public ReceiptController(IConfiguration configuration,IHostingEnvironment hostingEnvironment)
        {
            objIPaymentlisting = new Paymentlisting(configuration);
            objIRecepit = new Receipt(configuration);
        ...
        public ActionResult Generatereceipt(RecepitDTO obj, string actionType)
        {
            ...
            if (string.Equals(actionType, "Download Receipt"))
            {
                ...
                else
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        rptH.FileName = _hostingEnvironment.MapPath("~/Reports/Recepit.rpt");
                        rptH.Load();
            ...
            else if (string.Equals(actionType, "Download Candidate Form"))
            {
                ...
                else
                {
                    ...
                    {
                        ReportClass rptH = new ReportClass();
                        rptH.FileName = _hostingEnvironment.MapPath("~/Reports/DetailsForm.rpt");
                        rptH.Load();
                        rptH.SetDataSource(ds.Tables[0]);
                        //Response.Buffer = false;
                        //Response.ClearContent();
