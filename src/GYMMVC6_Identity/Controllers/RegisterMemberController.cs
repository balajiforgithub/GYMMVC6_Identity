...
namespace GYMONE.Controllers
{
    ...
    public class RegisterMemberController : Controller
    {
        ...
        public RegisterMemberController(IConfiguration configuration)
        {
            objscheme = new SchemeMaster(configuration);
            objPlan = new PlanMaster(configuration);
            objRegMem = new RegisterMember(configuration);
            objpay = new PaymentDetails(configuration);
        }

        [HttpGet]
        public ActionResult Index()
        {
            var memberDtos = objRegMem.GetMember();
        ...
        public ActionResult Create(MemberRegistrationDTO obj, FormCollection frm)
        {
            try
            {
                ...
                else
                {
                    ...
                    obj.Createdby = Convert.ToInt32(ViewData["UserID"]);
                    ...
                    if (MemberID > 0)
                    {
                        ...
                        if (payresult > 0)
                        {
                            ViewData["Message"] = "Member Created Successfully.";
                        }
                        else
                        {
                            ViewData["Message"] = "Some thing went wrong while Member Created .";
                        }
                    }
                    else
                    {
        ...
        public ActionResult Edit(MemberRegistrationDTO objMR, string actionType)
        {
            if (actionType == "Update")
            {
                ...
                else
                {
                    ...
                    objMR.Createdby = Convert.ToInt32(ViewData["UserID"]);
        ...
        public int Pay(MemberRegistrationDTO obj, int MemberID)
        {
            try
            {
                ...
                PayPD.CreateUserID = Convert.ToInt32(ViewData["UserID"]);
