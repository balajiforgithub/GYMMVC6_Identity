...
namespace GYMONE.Controllers
{
    ...
    public class RenewalController : Controller
    {
        ...
        public RenewalController(IConfiguration configration)
        {
            objscheme = new SchemeMaster(configration);
            objPlan = new PlanMaster(configration);
            objRegMem = new RegisterMember(configration);
            objpay = new PaymentDetails(configration);
            objIPaymentlisting = new Paymentlisting(configration);
            objRenewal = new Renewal(configration);
        }
        ...
        public int Pay(RenewalDATA obj)
        {
            try
            {
                ...
                PayPD.CreateUserID = Convert.ToInt32(TempData["UserID"]);
