...
using FormCollection = Microsoft.AspNet.Http.Internal.FormCollection;
...
namespace GYMONE.Controllers
{
    ...
    //////[MyExceptionHandler]
    [Microsoft.AspNet.Authorization.Authorize(Roles = "SystemUser,Admin")]
    public class YearwiseCollectionController : Controller
    {
        IlReports objIlReports;
        private readonly IHostingEnvironment _hostingEnvironment;
        public YearwiseCollectionController(IConfiguration connection,IHostingEnvironment hostingEnvironment)
        {
            objIlReports = new ReportsMaster(connection);
        ...
        public ActionResult YearwiseReport(FormCollection FC, YearwiseModel objMW)
        {
            ...
            else
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    rptH.FileName = _hostingEnvironment.MapPath("~/Reports/YearwiseCollectionreport.rpt");
                    rptH.Load();
                    rptH.SetDataSource(ds.Tables[0]);
                    ////NOTE : Not implemented in Asp.Net 5
                    ////if (Response != null)
                    ////{
                    ////    new ViewContext().HttpContext.Response.
